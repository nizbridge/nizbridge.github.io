...
- 2026-07 안정형 전체 셋업 데이터베이스 정규화 JSON
- LMGT3와 Hypercar의 트랙별 안정형 `.svm` 참조 셋업(368개)
- 참조 셋업 매니페스트

기준 데이터는 `data/normalized/lmu-setup-db-2026-07.json`이다. 이 파일은 이전 엑셀 데이터베이스의 7개 시트를 모두 보존한다.

## 다음 개발 단계

1. 정규화 JSON과 `.svm` 필드를 분석해 표준 데이터 모델을 확장합니다.
