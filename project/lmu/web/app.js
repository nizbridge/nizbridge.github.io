...
function renderSetups() {
  ...
  visible.forEach((setup) => { const card = $("#setup-card").content.cloneNode(true); $(".class-badge", card).textContent = setup.class; $(".confidence", card).textContent = `신뢰도 ${setup.confidence}`; $("h2", card).textContent = setup.vehicle; $(".track", card).textContent = setup.track; $(".template", card).textContent = setup.template_vehicle; $(".status", card).textContent = setup.validation_status === "REVIEW_V142" ? "V1.4.2 주행 재검증 필요" : "검증 필요"; const link = $(".download", card); link.href = `../data/reference-setups/2026-09-v1.4.2/${encodeURI(setup.file)}`; link.download = setup.file.split("/").pop(); grid.append(card); });
