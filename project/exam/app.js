...
function renderVocabulary() { const lesson = lessons.find(item => item.id === vocabularyLessonId); els.vocabularyTabs.innerHTML = lessons.map(item => `<button class="vocabulary-tab ${item.id === vocabularyLessonId ? 'active' : ''}" data-vocabulary-lesson="${item.id}">${item.title}<small>${item.words.length}</small></button>`).join(''); els.wordList.innerHTML = lesson.words.map(word => `<div class="word-row"><b>${word.english}</b><button class="speaker" data-pronounce="${word.english}" aria-label="${word.english} 발음 듣기">🔊</button><span>${word.korean}</span></div>`).join(''); }
function speak(word) { if (!('speechSynthesis' in window)) return; window.speechSynthesis.cancel(); const utterance = new SpeechSynthesisUtterance(word.replace(/[()]/g, '').replace(/\(e\)/, 'e')); utterance.lang = 'en-US'; utterance.rate = 0.85; window.speechSynthesis.speak(utterance); }
function addPronunciationButton(container, word) { const button = document.createElement('button'); button.className = 'speaker feedback-speaker'; button.dataset.pronounce = word; button.setAttribute('aria-label', `${word} 발음 듣기`); button.textContent = '🔊'; container.append(' ', button); }
...
document.addEventListener('click', event => { const speaker = event.target.closest('[data-pronounce]'); if (speaker) { speak(speaker.dataset.pronounce); return; } const vocabularyTab = event.target.closest('[data-vocabulary-lesson]'); if (vocabularyTab) { vocabularyLessonId = vocabularyTab.dataset.vocabularyLesson; renderVocabulary(); return; } const button = event.target.closest('[data-view]'); if (button) { switchView(button.dataset.view); if (button.dataset.view === 'vocabulary') renderVocabulary(); } });
