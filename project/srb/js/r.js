...
function raceLoop(t) {
  ...
  G.cars.forEach(car => {
    ...
    if (isCorner) {
      if (car.batMode === 'B') {
        car.battery = Math.max(0, car.battery - BAT_DRAIN_BOOST * dt);
        if (car.battery <= 0) car.batMode = 'N';
      } else {
  ...
  const rows = sorted.map((car,i) => {
    ...
    return `
      <tr class="${car.isPlayer?'player-row':''}">
        ...
        <td>${car.d.team}</td>
        <td class="td-time"${isFL?' style="color:#c77dff;font-weight:700"':''}>${best?fmtTime(best):'—'}</td>
      </tr>`;
  }).join('');
  ...
  document.getElementById('results-table').innerHTML = `
    <div class="results-tbl-wrap">
      <table class="results-tbl">
        <thead><tr><th>POS</th><th>드라이버</th><th>팀</th><th>BEST LAP</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
    </div>`;
}
